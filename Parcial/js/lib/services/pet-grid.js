...
export class PetGrid extends Grid {
    constructor(...cols) {
        super(cols);
    }
    generate() {
    ...
    search(event) {
        ...
        trs.forEach(tr => {
            ...
            for (let i = 0; i < tds.length; i++) {
                if (tds[i].innerHTML.match(event.target.value) === null) {
    ...
    actionFor(event) {
        ...
        for (let i = 0; i < options.length; i++) {
            if (options[i].selected === true) {
                switch (event.target.value + "-" + options[i].value) {
                    case "submit-catto":
                    ...
                    case "submit-birdo":
                        ...
                        let birdoType = document.getElementById("pet-eType").getElementsByTagName("option");
...
//# sourceMappingURL=pet-grid.js.map